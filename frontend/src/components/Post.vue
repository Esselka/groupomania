...
<template>
  <article class="card mb-3 mx-auto">
    ...
    <div class="card-body">
      ...
      <p class="card-text text-left text-muted mt-2">{{ this.getPoints }} points · <span v-if="this.$route.name === 'MainPage'" type="button" @click="goToPost(post.slug)">{{ post.commentsNumber }} commentaires</span><span v-if="this.$route.name === 'PostID'">{{ post.commentsNumber }} commentaires</span></p>
